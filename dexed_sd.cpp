...
{
#ifdef DEBUG
  Serial.printf_P(PSTR("save sd voice, b:%d - d:%d\n"), b, v);
#endif
  v = constrain(v, 0, MAX_VOICES - 1);
  b = constrain(b, 0, MAX_BANKS - 1);
  ...
  if (sd_card > 0)
  {
    ...
    snprintf_P(filename, strlen(filename), PSTR("/%d/%s.syx"), b, g_bank_name[instance_id]);
...
FLASHMEM bool load_sd_drummappings_json(uint8_t number)
{
  ...
  if (sd_card > 0)
  {
    ...
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, DRUMS_MAPPING_NAME);
...
FLASHMEM bool load_sd_microsynth_json(uint8_t ms, uint8_t instance_id)
{
  ...
  if (sd_card > 0)
  {
    ...
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s%d.json"), PERFORMANCE_CONFIG_PATH, ms, MICROSYNTH_CONFIG_NAME, instance_id + 1);
...
FLASHMEM bool load_sd_braids_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, BRAIDS_CONFIG_NAME);
...
FLASHMEM bool load_sd_multiband_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, MULTIBAND_CONFIG_NAME);
...
FLASHMEM bool load_sd_chain_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, CHAIN_CONFIG_NAME);
    // first check if file exists...
    AudioNoInterrupts();
    if (SD.exists(filename))
    {
...
FLASHMEM bool load_sd_transpose_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, TRANSPOSE_CONFIG_NAME);
    // first check if file exists...
    AudioNoInterrupts();
    if (SD.exists(filename))
    {
...
FLASHMEM bool load_sd_song_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, SONG_CONFIG_NAME);
    // first check if file exists...
    AudioNoInterrupts();
    if (SD.exists(filename))
    {
...
FLASHMEM bool save_sd_seq_sub_vel_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, VELOCITY_CONFIG_NAME);
#ifdef DEBUG
...
FLASHMEM bool save_sd_seq_sub_patterns_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, PATTERN_CONFIG_NAME);
#ifdef DEBUG
...
FLASHMEM bool save_sd_performance_json(uint8_t number)
{
  ...
  if (sd_card > 0)
  {
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, SEQUENCER_CONFIG_NAME);
#ifdef DEBUG
...
FLASHMEM bool get_bank_name(uint8_t b, char* bank_name) {
  ...
  if (sd_card > 0)
  {
    ...
    snprintf_P(bankdir, strlen(filename), PSTR("/%d"), b);
...
FLASHMEM bool save_sd_multisample_presets_json(uint8_t number)
{
  ...
  {
    number = constrain(number, PERFORMANCE_NUM_MIN, PERFORMANCE_NUM_MAX);
    snprintf_P(filename, strlen(filename), PSTR("/%s/%d/%s.json"), PERFORMANCE_CONFIG_PATH, number, MULTISAMPLE_PRESETS_CONFIG_NAME);
#ifdef DEBUG
