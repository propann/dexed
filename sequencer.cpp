...
void sequencer_part1(void) {
  ...
  for (uint8_t d = 0; d < NUM_SEQ_TRACKS; d++) {
    ...
    {
      if (seq.track_type[d] == 0 && seq.ticks == 0) {  // drum track (drum samples and pitched one-shot samples)
#if NUM_DRUMS > 0
        ...
        if (seq.note_data[seq.current_pattern[d]][seq.step] > 0) {
          if (seq.track_type[d] == 1)  // instrument track
          {
            if (seq.note_data[seq.current_pattern[d]][seq.step] != 130 && seq.ticks == 0) {
              ...
              else if (seq.instrument[d] > 5 && seq.instrument[d] < 16)  // track is for internal MultiSampler
              {
                if (check_probability(seq.current_pattern[d]))
        ...
        if ((seq.track_type[d] == 3 && seq.arp_num_notes_count < seq.arp_num_notes_max && seq.euclidean_active == false) || (seq.track_type[d] == 3 && seq.arp_num_notes_count < seq.arp_num_notes_max && seq.euclidean_active && seq.euclidean_state[seq.step])) {  //Arp
...
void sequencer(void) {  // Runs in Interrupt Timer. Switches between the Noteon and Noteoff Task, each cycle
