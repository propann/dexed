...
//
#ifndef TEENSY_RESAMPLING_QSPIREADER_WAVEHEADERPARSER_H
#define TEENSY_RESAMPLING_QSPIREADER_WAVEHEADERPARSER_H
...
public:
    bool readWaveHeader(const char *filename, wav_header &header) {
        __disable_irq();
        File wavFile = myfs.open(filename);
        __enable_irq();
        if (!wavFile) {
...
#endif //TEENSY_RESAMPLING_QSPIREADER_WAVEHEADERPARSER_H
