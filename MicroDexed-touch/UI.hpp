...
LCDML_add(23, LCDML_0_8, 5, "Live Seq.Edit", UI_func_liveseq_listeditor);
LCDML_add(24, LCDML_0_8, 6, "Live Seq.GrphEdit", UI_func_liveseq_pianoroll);
...
LCDML_add(30, LCDML_0_8, 12, "Clear ALL", UI_func_clear_all);
LCDML_add(31, LCDML_0_8, 13, "Clear Song", UI_func_clear_song);
LCDML_add(32, LCDML_0_8, 14, "Clear Chains", UI_func_clear_song_chains);
LCDML_add(33, LCDML_0_8, 15, "Clear Patterns", UI_func_clear_patterns);
LCDML_add(34, LCDML_0_8, 16, "Settings", UI_func_seq_settings);
LCDML_add(35, LCDML_0, 9, "Mixer", UI_func_mixer);
LCDML_add(36, LCDML_0, 11, "Sample Editor", UI_func_sample_editor);
LCDML_add(37, LCDML_0, 12, "Audio/SMP Record", UI_func_recorder);
LCDML_add(38, LCDML_0, 13, "Load/Save", NULL);
LCDML_add(39, LCDML_0_13, 1, "Load Perf.", UI_func_load_performance);
LCDML_add(40, LCDML_0_13, 2, "Save Perf.", UI_func_save_performance);
LCDML_add(41, LCDML_0_13, 3, "Name Perf.", UI_func_set_performance_name);
LCDML_add(42, LCDML_0_13, 4, "Name Multisample", UI_func_set_multisample_name);
LCDML_add(43, LCDML_0_13, 5, "MIDI", NULL);
LCDML_add(44, LCDML_0_13_5, 1, "MIDI Recv Bank", UI_func_sysex_receive_bank);
LCDML_add(45, LCDML_0_13_5, 2, "MIDI Snd Bank", UI_func_sysex_send_bank);
LCDML_add(46, LCDML_0_13_5, 3, "MIDI Snd Voice", UI_func_sysex_send_voice);
LCDML_add(47, LCDML_0, 14, "File Manager", UI_func_file_manager);
LCDML_add(48, LCDML_0, 15, "System", NULL);
LCDML_add(49, LCDML_0_15, 1, "Stereo/Mono", UI_func_stereo_mono);
LCDML_add(50, LCDML_0_15, 2, "MIDI channels", UI_func_midi_channels);
LCDML_add(51, LCDML_0_15, 3, "MIDI Soft THRU", UI_func_midi_soft_thru);
LCDML_add(52, LCDML_0_15, 4, "MIDI Mapping", UI_func_custom_mappings);
...
FLASHMEM void UI_func_chorus_frequency(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    chorus_modulator[selected_instance_id]->frequency(configuration.fx.chorus_frequency[selected_instance_id] / 20.0);
  }
...
FLASHMEM void UI_func_chorus_depth(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    chorus_modulator[selected_instance_id]->amplitude(configuration.fx.chorus_depth[selected_instance_id] / 1000.0);
  }
...
FLASHMEM void UI_func_dexed_audio(uint8_t param)
{
  ...
  if (LCDML.FUNC_setup()) // ****** SETUP *********
  {
    ...
    ui.addEditor("DEPTH", CHORUS_DEPTH_MIN, CHORUS_DEPTH_MAX, &configuration.fx.chorus_depth[0],
      ...
      {
        fx_current_instance_setter(editor, value);
        chorus_modulator[selected_instance_id]->amplitude(value / 1000.0); });
...
FLASHMEM void UI_func_seq_settings(uint8_t param)
{
  ...
  if (LCDML.FUNC_setup()) // ****** SETUP *********
  {
    ...
    setCursor_textGrid_small(1, 1);
    display.print("SEQUENCER SETTINGS");
    ...
    setCursor_textGrid_small(27, 11);
    display.print("-");
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    if (seq.transpose != 0)
    {
      ...
      display.print(displayname);
      setCursor_textGrid_small(29, 11);
...
uint8_t liveseq_pianoroll_eventcount = 0;
int xscaler = 33;
uint8_t xoff = 33;
float pat_len = 0;
uint8_t liveseq_pianoroll_lowest_note = 254;
...
FLASHMEM void liveseq_get_lowest_note()
{
  std::vector<LiveSequencer::NotePair> notePairs = liveSeq.getNotePairsFromTrack(temp_int);

  // find lowest note
  liveseq_pianoroll_lowest_note = 254;
  int i = 0;
  if (notePairs.size() != 0)
  {
    for (uint16_t j = 0; j < notePairs.size(); j++)
    {
      if (notePairs[j].noteOn.track == temp_int && notePairs[j].noteOn.event == midi::NoteOn)
        if (notePairs[j].noteOn.note_in < liveseq_pianoroll_lowest_note && notePairs[j].noteOn.note_in> 0)
        {
          liveseq_pianoroll_lowest_note = notePairs[j].noteOn.note_in;
        }
      i++;
    }
  }
  liveseq_pianoroll_y_scroll = liveseq_pianoroll_lowest_note;
  // if (liveseq_pianoroll_lowest_note > 120)
  //   liveseq_pianoroll_lowest_note = 24;
  //  else   if (liveseq_pianoroll_lowest_note < 12)
  //   liveseq_pianoroll_lowest_note = 12;

}

FLASHMEM uint8_t get_octave(uint8_t in)
{
  if (in % 12 == 0)
    return in / 12;
  else
...
FLASHMEM  void liveseq_pianoroll_draw_graphics()
{
  ...
  if (menuhelper_redraw || generic_menu == 2 || generic_menu == 4 || generic_menu == 5 || generic_menu == 20 || generic_menu == 21)
  {
    ...
    for (uint8_t j = 0; j < 4; j++)
    {
      if (liveseq_pianoroll_x_scroll + xoff + j * pat_len >= xoff)
        display.drawLine(liveseq_pianoroll_x_scroll + xoff + j * pat_len, CHAR_height, liveseq_pianoroll_x_scroll + xoff + j * pat_len, DISPLAY_HEIGHT - 5 * CHAR_height + 5, GREY1);

      for (uint8_t k = 1; k < 16; k++)
      {
        if (liveseq_pianoroll_x_scroll + xoff + j * pat_len + pat_len / 16 * k >= xoff)
          display.drawLine(liveseq_pianoroll_x_scroll + xoff + j * pat_len + pat_len / 16 * k, CHAR_height, liveseq_pianoroll_x_scroll + xoff + j * pat_len + pat_len / 16 * k, DISPLAY_HEIGHT - 5 * CHAR_height + 5, GREY2);
      }
    }
    if (liveseq_pianoroll_x_scroll + xoff + 4 * pat_len >= xoff)
      display.drawLine(liveseq_pianoroll_x_scroll + xoff + 4 * pat_len, CHAR_height, liveseq_pianoroll_x_scroll + xoff + 4 * pat_len, DISPLAY_HEIGHT - 5 * CHAR_height + 5, GREY1);

  }

  if (notePairs.size() != 0)
  {
    // liveseq_get_lowest_note();
    if (generic_menu == 2)
    {
      //get current value when starting editing so it does not start at zero
      if (liveseq_pianoroll_get_current)
      {
        temp_uint = notePairs[generic_temp_select_menu].noteOn.note_in;
        liveseq_pianoroll_get_current = false;
      }
  ...
  if (notePairs.size() != 0)
  {
    ...
    if (liveseq_pianoroll_eventcount > 0)
      //for (uint8_t j = 0; j < liveseq_pianoroll_eventcount; j++)
      for (int j = liveseq_pianoroll_eventcount - 1; j > 0; j--)
      {
        uint16_t col;

        // if (notePairs[j].noteOn.note_in == notePairs[j+1].noteOn.note_in && notePairs[j].noteOn.patternMs == notePairs[j+1].noteOn.patternMs  )
        // col = YELLOW;
        // else
        // if ( j-1>=0 && notePairs[j].noteOn.note_in == notePairs[j-1].noteOn.note_in &&  notePairs[j].noteOn.patternMs == notePairs[j+1].noteOn.patternMs  )
        // col = PINK;
        // else

        if (generic_menu > 1 && generic_menu != 99 && j == generic_temp_select_menu)
        {
          col = RED;
        }
        else if (j == generic_temp_select_menu && generic_menu > 0)
        {
          col = GREEN;
        }
        else
        ...
        if (DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * notePairs[j].noteOn.note_in) >= CHAR_height &&
          DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * notePairs[j].noteOn.note_in) < DISPLAY_HEIGHT - 5 * CHAR_height &&
          liveseq_pianoroll_x_scroll + notePairs[j].noteOn.patternNumber * pat_len + xoff + notePairs[j].noteOn.patternMs / xscaler >= xoff &&
          liveseq_pianoroll_x_scroll + notePairs[j].noteOn.patternNumber * pat_len + xoff + notePairs[j].noteOn.patternMs / xscaler < DISPLAY_WIDTH - 5
          )
        {
          if (notePairs[j].noteOff.note_in > 0 && notePairs[j].noteOn.note_in > 0)
          {
            display.console = true;
            display.fillRect(liveseq_pianoroll_x_scroll + notePairs[j].noteOn.patternNumber * pat_len + xoff + notePairs[j].noteOn.patternMs / xscaler,
              (DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * notePairs[j].noteOn.note_in)),
          ...
          else //no note-off for note-on found, draw just note start
          {
            if (generic_menu == 1 && j == generic_temp_select_menu)
              ...
              col = GREEN;
            display.console = true;
            display.fillRect(liveseq_pianoroll_x_scroll + notePairs[j].noteOn.patternNumber * pat_len + xoff + notePairs[j].noteOn.note_in / xscaler,
              (DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * notePairs[j].noteOn.note_in)), 5, 5, col);
          }
        }
      }
  }
...
FLASHMEM void print_pianokeys_liveseq()
{
  //liveseq_get_lowest_note();
  ...
  print_keyboard_livesequencer((8.15 * liveseq_pianoroll_y_scroll + (7 * 8.15) - 2) - 196 * 2, get_octave(liveseq_pianoroll_lowest_note) + 2);
  print_keyboard_livesequencer((8.15 * liveseq_pianoroll_y_scroll + (7 * 8.15) - 2) - 196, get_octave(liveseq_pianoroll_lowest_note));
  print_keyboard_livesequencer((8.15 * liveseq_pianoroll_y_scroll + (7 * 8.15) - 2), get_octave(liveseq_pianoroll_lowest_note) - 2);

  display.console = true;
  display.fillRect(0, CHAR_height_small, 30, CHAR_height_small + 1, COLOR_BACKGROUND);
}

bool liveseq_note_copy_state = false;
...
FLASHMEM  void buttons_liveseq_pianoroll()
{
  ...
  if (generic_menu == 23) // scroll right/left
  {
    draw_button_on_grid(16, 21, "SCROLL", itoa(liveseq_pianoroll_x_scroll, buf, 10), 2);
  }
  else
    if (liveseq_pianoroll_fullrefresh_values)
      draw_button_on_grid(16, 21, "SCROLL", "R/L", 1);

  if (generic_menu == 22) // zoom x
  {
    draw_button_on_grid(24, 21, "ZOOM", itoa(33 - xscaler, buf, 10), 2);
  }
  else
    if (liveseq_pianoroll_fullrefresh_values)
      draw_button_on_grid(24, 21, "ZOOM", "X", 1);
  ...
  if (generic_menu == 2 && notePairs.size() != 0) // edit note
  {
  ...
  if (generic_menu == 24) // copy (add) new note
  {
    //note add
    ...
    liveseq_note_copy_state = true;
    draw_button_on_grid(48, 21, "PLACE", "COPY", 3);
    generic_menu = 2;
  }
  else if (generic_menu == 1)
    draw_button_on_grid(48, 21, "COPY", "NOTE", 1);
  else if (generic_menu == 2 && liveseq_note_copy_state)
    draw_button_on_grid(48, 21, "SAVE", "COPY", 3);
  else if (liveseq_pianoroll_fullrefresh_values || liveseq_note_copy_state == false)
    draw_button_on_grid(48, 21, "COPY", "NOTE", 0);
  ...
  if (generic_menu == 4 && notePairs.size() != 0) // edit start
  {
  ...
  if (generic_menu == 5 && notePairs.size() != 0) // edit end
  {
  ...
  if (generic_menu == 21 && notePairs.size() != 0) // delete note
  {
  ...
  else
  {
    ...
    if (generic_menu == 99 && notePairs.size() != 0)
      liveseq_pianoroll_fullrefresh_values = true;
    else
      liveseq_pianoroll_fullrefresh_values = false;
  }
}
...
{  // for Livesequencer
  ...
  if (LCDML.FUNC_setup()) // ****** SETUP *********
  {
    ...
    {
      liveseq_get_lowest_note();
      // liveseq_pianoroll_y_scroll = get_octave(liveseq_pianoroll_lowest_note);
    }
    // setup function
    display.fillScreen(COLOR_BACKGROUND);
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    else if (generic_menu == 2) // edit note
    {
      if (LCDML.BT_checkDown())
      {
        ...
        if (int(DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * temp_uint)) < 35)
        {
          liveseq_pianoroll_y_scroll++;
          print_pianokeys_liveseq();
        }
      ...
      {
        temp_uint = constrain(temp_uint - 1, 1, 127);
        if (int(DISPLAY_HEIGHT + (8.15 * liveseq_pianoroll_y_scroll) - (8.15 * temp_uint)) > 140)
        {
          liveseq_pianoroll_y_scroll--;
          print_pianokeys_liveseq();
        }
    ...
    else if (generic_menu == 20) // scroll up&down
    {
      if (LCDML.BT_checkDown())
      {
        liveseq_pianoroll_y_scroll = constrain(liveseq_pianoroll_y_scroll + 1, -60, 36 + 24);
      }
      else if (LCDML.BT_checkUp())
      {
        liveseq_pianoroll_y_scroll = constrain(liveseq_pianoroll_y_scroll - 1, -60, 36 + 24);
    ...
    else if (generic_menu == 22) // scale x
    {
      if (LCDML.BT_checkDown())
      {
        menuhelper_previous_val = xscaler;
        xscaler = constrain(xscaler - 1, 10, 33);  // -1 reversed

        if (menuhelper_previous_val != xscaler)
        {

          liveseq_pianoroll_x_scroll = liveseq_pianoroll_x_scroll * 0.6;
          menuhelper_redraw = true;
        }
      }
      else if (LCDML.BT_checkUp())
      {
        ...
        if (menuhelper_previous_val != xscaler)
        {
          liveseq_pianoroll_x_scroll = liveseq_pianoroll_x_scroll / 1.2;
          menuhelper_redraw = true;
        }
      }
    }

    else if (generic_menu == 23) // scroll right/left
    {
      if (LCDML.BT_checkDown())
      {
        liveseq_pianoroll_x_scroll = constrain(liveseq_pianoroll_x_scroll + 1 + 33 - xscaler, -599, 599);
        menuhelper_redraw = true;
      }
      else if (LCDML.BT_checkUp())
      {
    ...
    if ((generic_menu == 0 && menuhelper_redraw) || generic_menu != 0)
...
FLASHMEM void print_psram_stats_filemanager()
{
  ...
  if (psram_size != 0)
  {
    sprintf(text1, "%05d MB", (int)psram_size / 1024);
    display.print(text1);
  }
  ...
  sprintf(text1, "%05d KB/%05d KB  USED", (int)total_data_size / 1024, (int)(psram_size / 1024));
  display.print(text1);
  display.setCursor(CHAR_width_small * 38, 6 * CHAR_height_small);
  sprintf(text1, "%05d KB  FREE", (int)(psram_size / 1024 - total_data_size / 1024));
  display.print(text1);
...
FLASHMEM void UI_func_file_manager(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    if (LCDML.BT_checkEnter() && fm.sd_mode == FM_PLAY_SAMPLE) // preview
    {
      preview_sample();
    }
    // show files
...
FLASHMEM void print_voice_settings_in_dexed_voice_select(bool fullrefresh_text, bool fullrefresh_values)
{
  ...
  if (fullrefresh_text)
  {
    ...
    if (seq.cycle_touch_element != 1)
    {
      ...
      setCursor_textGrid_small(19, 14);
      display.print(F("EDIT"));

      setCursor_textGrid_small(19, 15);
      display.print(F("EDIT"));

      setCursor_textGrid_small(19, 16);
      display.print(F("EDIT"));
  ...
  if (seq.cycle_touch_element != 1)
  {
    ...
    if (menu_item_check(10) || fullrefresh_values)
      print_small_intbar(9, 14, configuration.fx.chorus_level[selected_instance_id], 10, 1, 1);
    if (menu_item_check(12) || fullrefresh_values)
      print_small_intbar(9, 15, configuration.fx.delay_level[selected_instance_id], 12, 1, 1);
    if (menu_item_check(14) || fullrefresh_values)
      print_small_intbar(9, 16, configuration.fx.reverb_send[selected_instance_id], 14, 1, 1);
...
FLASHMEM void UI_func_voice_select(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop()) // ****** LOOP *********
  {
    ...
    if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up()))
    {
      ...
      if (seq.edit_state == 0)
      {
        generic_temp_select_menu = calc_val[state_dir](generic_temp_select_menu, 1, 0, 16);
        if (generic_temp_select_menu != menuhelper_previous_val)
          menuhelper_redraw = true;
      }
      else if (seq.edit_state == 1)
      {
        ...
        if (generic_temp_select_menu == 12) // DELAY
        {
        ...
        if (generic_temp_select_menu == 14) // REVERB SEND
        {
        ...
        if (generic_temp_select_menu == 16) // BPM
        {
    ...
    {
      if (generic_temp_select_menu == 11 && seq.cycle_touch_element == 0)
      {
        //chorus edit 
        LCDML.OTHER_jumpToFunc(UI_func_dexed_audio);
      }
      else if (generic_temp_select_menu == 13 && seq.cycle_touch_element == 0)
      {
        //delay edit
        LCDML.OTHER_jumpToFunc(UI_func_dexed_audio);
      }
      else if (generic_temp_select_menu == 15 && seq.cycle_touch_element == 0)
      {
        //reverb edit
