...
public:
  ...
  // const uint8_t sizes[P_TYPE_COUNT]={0,sizeof(uint8_t),sizeof(uint16_t),sizeof(uint32_t),sizeof(int32_t),sizeof(float)};
...
public:
  ...
  float get() {
    ...
    if(desc->type == Descriptor::P_UINT32_T) return *((uint32_t*)ptr);
  ...
  void set(float value) {
    ...
    if(desc->type == Descriptor::P_INT32_T) *((int32_t*)ptr) = round(value);
...
#define P_bool(name,def) \
  Param name##_desc{Descriptor::P_UINT8_T, 1, #name, def}; uint8_t name = def

#define P_uint8_t(name,min,max,def) \
  Param name##_desc{Descriptor::P_UINT8_T, 1, #name, min, max, def}; uint8_t name = def

#define P_uint16_t(name,min,max,def) \
  Param name##_desc{Descriptor::P_UINT16_T, 1, #name,min,max,def}; uint16_t name = def

#define P_int32_t(name,min,max,def) \
  Param name##_desc{Descriptor::P_INT32_T, 1, #name,min,max,def}; int32_t name = def

#define P_uint32_t(name,min,max,def) \
  Param name##_desc{Descriptor::P_UINT32_T, 1, #name,min,max,def}; int32_t name = def

#define P_float(name,min,max,def) \
  Param name##_desc{Descriptor::P_FLOAT, 1, #name,min,max,def}; float name = def

#define P_end Param _p_end_desc{Descriptor::P_END};
