...
int levelmeter;
int []peak= new int[20];
int []levelmeter_x= new int[20];
int []levelmeter_y= new int[20];
...
void update_volmeter(int x, int y, int h, int meter) {

  levelmeter_x[ meter] =x;
  levelmeter_y[ meter] =y;
  levelmeter_h[meter]= h;
...
void draw_volmeters() {
  ...
  for (int j = 0; j < num_meters; j++)
  {
    if (  levelmeter_h[j] > peak[j]) {
      int z = 0;
      do {
        stroke( toRGB ((100-levelmeter_h[j] + z), 200, 200));
        line(levelmeter_x[j], levelmeter_y[j] -levelmeter_h[j]+z, levelmeter_x[j]+17 *2, levelmeter_y[j] -levelmeter_h[j]+z );
        z=z+1;
      } while (z < levelmeter_h[j] - peak[j]);
      peak[j] = levelmeter_h[j];
    } else
    {
      ...
      {
        stroke(0);
        rect(levelmeter_x[j], levelmeter_y[j] - (peak[j]), 17*2, 1);
        peak[j] = peak[j] - 1;
      }
    }
  }
...
void read_and_set_circle_color()
{
  colh= myPort.read();
  coll= myPort.read();
  col =  colh*256+coll;
