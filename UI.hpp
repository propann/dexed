...
{
  //  if (seq.active_function == 1 && seq.menu == 19) // edit content type of current pattern
  //  {
  //    if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up()))
  //    {
  //      if (LCDML.BT_checkDown())
  ...
  if (seq.active_function == 1 && seq.menu == 19) // edit auto/manual advance in step recorder
  {
    if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up()))
    {
      if (LCDML.BT_checkDown())
...
draw_button_on_grid(45, 20, "", "", 0);
  ...
  if (LCDML.FUNC_loop())          // ****** LOOP *********
  {
    ...
    if (seq.active_function == 99 && seq.menu > 18) // sub_menus
    {
      if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up()))
      {
        if (LCDML.BT_checkDown())
    ...
    if (seq.active_function == 99 && seq.menu < 17)
    {
      if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up()))
      {
        if (LCDML.BT_checkDown())
    ...
    if ( seq.note_data[seq.active_pattern][seq.menu - 1] > 0 )
    {
      ...
      {
        if (seq.active_function == 1 && seq.content_type[seq.active_pattern] < 2 )
        ...
        else  if (seq.active_function == 1 && seq.content_type[seq.active_pattern] > 1 )
    ...
    if (LCDML.BT_checkEnter())  //handle button presses during menu >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    {
      ...
      else if (seq.menu > 0 && seq.menu < 19 && seq.active_function == 99)
      {
      ...
      else if (seq.menu > 18 && seq.menu < 20 && seq.active_function == 99)
      {
        seq.active_function = 1;
      }
      else
    ...
    if (seq.menu == 19 ) //edit content type of pattern
    {
      ...
      if (seq.active_function != 1)
      {
        ...
        display.setTextSize(1);
        display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
        display.print(F("CHANGE STEP RECORDING MODE?  "));
        display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
      } else
      {
        display.setTextSize(1);
        ...
        display.setTextColor( COLOR_BACKGROUND, RED);
        display.print(F("EDIT REC. MODE WITH ENC_R.  "));
        display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
      }
      if (seq.active_function == 1)
        display.setTextColor( RED, COLOR_BACKGROUND);
...
void print_current_sample_and_pitch_buffer()
{
  if (seq.cycle_touch_element == 0)  // touch keyboard is off
  {
    ...
    print_performance_name(CHAR_width_small * 36 , 13 * (CHAR_height_small + 2) + 10);
...
void UI_func_microsynth(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop())          // ****** LOOP *********
  {
    ...
    if (generic_temp_select_menu == 25)
    {
      display.setTextColor(COLOR_BACKGROUND, COLOR_SYSTEXT);
    } else display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
...
void flash_printDirectory(File flash_currentDirectory) //QSPI
{
  ...
  fm.flash_cap_rows = 9;
  for (uint8_t f = 0; f < fm.flash_skip_files; f++)
  {
    fm.flash_entry.close();
    fm.flash_entry = flash_currentDirectory.openNextFile();

    if (! fm.flash_entry)  break;
  }
  if (fm.flash_parent_folder && fm.flash_folder_depth > 0)
  {
    fm.flash_is_folder = true;
    strcpy(fm.flash_temp_name, ".." );
    display.setTextColor(COLOR_SYSTEXT, COLOR_PITCHSMP);
  }
  else
    display.setTextColor(COLOR_PITCHSMP, COLOR_BACKGROUND);

  if ( fm.flash_folder_depth > 0)
  {
    drawBitmap(CHAR_width + DISPLAY_WIDTH / 2,  5 * 11 - 1, special_chars[23], 8, 8, YELLOW);

    display.setCursor (CHAR_width * 2 + DISPLAY_WIDTH / 2,   + 5 * 11  );
    display.print("..");
  }
  else
  {
    drawBitmap(CHAR_width + DISPLAY_WIDTH / 2,  5 * 11 - 1, special_chars[23], 8, 8, COLOR_BACKGROUND);
    display.setCursor (CHAR_width * 2 + DISPLAY_WIDTH / 2,   + 5 * 11  );
    display.print("/ ");
  }
  ...
  for (uint8_t f = 0; f < 10; f++)
  {
    ...
    {
      fm.flash_cap_rows = f - 1;
      // display.fillRect(10 + DISPLAY_WIDTH/2, f * 11   + 6 * 11 - 1 , CHAR_width * 15, (10 - f) * 11, COLOR_BACKGROUND);
      break;
    }
    if (fm.flash_entry.isDirectory() )
    {
      drawBitmap(CHAR_width + DISPLAY_WIDTH / 2, f * 11 - 1 + 6 * 11 , special_chars[23], 8, 8, YELLOW);
      if (f == fm.flash_selected_file && fm.flash_parent_folder == false)
        display.setTextColor(COLOR_BACKGROUND, COLOR_PITCHSMP);
      else
        display.setTextColor(COLOR_PITCHSMP, COLOR_BACKGROUND);
      show_smallfont_noGrid( f * 11  + 6 * 11, CHAR_width * 2 + DISPLAY_WIDTH / 2, 14, fm.flash_entry.name() );
      display.setCursor (CHAR_width * 12 + 240, f * 11  + 6 * 11  );
      display.setTextColor(DX_DARKCYAN, COLOR_BACKGROUND);
      display.print("DIR    ");
    } else
    {
      drawBitmap(CHAR_width, f * 11 - 1 + 6 * 11 + DISPLAY_WIDTH / 2, special_chars[23], 8, 8, COLOR_BACKGROUND);
      if (f == fm.flash_selected_file && fm.flash_parent_folder == false)
        display.setTextColor(COLOR_BACKGROUND, COLOR_SYSTEXT);
      else
        display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
      show_smallfont_noGrid(f * 11  + 6 * 11, CHAR_width * 2 + DISPLAY_WIDTH / 2, 14, fm.flash_entry.name() );
      display.setTextColor(COLOR_DRUMS, COLOR_BACKGROUND);
      display.setCursor (CHAR_width_small * 19 + DISPLAY_WIDTH / 2, f * 11  + 6 * 11 );
      display.print(fm.flash_entry.size(), DEC);
      display.print(" ");
    }
...
void UI_func_MultiSamplePlay(uint8_t param)
{
  ...
  if (LCDML.FUNC_loop())          // ****** LOOP *********
  {
    ...
    for (uint8_t y = 0; y < NUM_MULTISAMPLE_ZONES; y++)
    {
      ...
      display.setTextColor(COLOR_SYSTEXT, COLOR_BACKGROUND);
      display.print(" ");
      setCursor_textGrid_small(14, y + yoffset);
...
void UI_func_file_manager(uint8_t param)
{
  ...
  if (LCDML.FUNC_setup())         // ****** SETUP *********
  {
    ...
    display.print(F("CURRENT FILE/FOLDER :"));
    print_file_manager_buttons();
    fm.flash_currentDirectory = myfs.open(fm.flash_new_name);
    //if (fm.flash_new_name[0] != 0x2f)
    //   fm.flash_new_name[0] = 0x2f;
    ...
    if (LCDML.BT_checkEnter() && fm.active_window == 0) // left window, SDCARD
    {
      if (fm.sd_mode == 4) //copy presets dir from SD to flash
      {
        ...
        while (1)
        {
          ...
          {
            // create the file on the Flash chip and copy data
            if  (myfs.exists(filename) == false)
            {
              File ff = myfs.open(filename, FILE_WRITE);
              if (ff)
              {
#ifdef DEBUG
                Serial.print(F("  copying"));
...
FLASHMEM void splash_draw_D() {
  ...
  display.fillRect( 3, 49, 25, 2, COLOR_PITCHSMP); display.fillRect( 83, 49, 27, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 54, 25, 2, COLOR_PITCHSMP); display.fillRect( 87, 54, 26, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 59, 25, 2, COLOR_PITCHSMP); display.fillRect( 90, 59, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 64, 25, 2, COLOR_PITCHSMP); display.fillRect( 92, 64, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 69, 25, 2, COLOR_PITCHSMP); display.fillRect( 94, 69, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 74, 25, 2, COLOR_PITCHSMP); display.fillRect( 95, 74, 24, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 79, 25, 2, COLOR_PITCHSMP); display.fillRect( 94, 79, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 84, 25, 2, COLOR_PITCHSMP); display.fillRect( 92, 84, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 89, 25, 2, COLOR_PITCHSMP); display.fillRect( 90, 89, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 94, 25, 2, COLOR_PITCHSMP); display.fillRect( 87, 94, 26, 2, COLOR_PITCHSMP);
  display.fillRect( 3, 99, 25, 2, COLOR_PITCHSMP); display.fillRect( 83, 99, 27, 2, COLOR_PITCHSMP);
...
FLASHMEM void splash_draw_reverseD() {
  ...
  display.fillRect( 291, 49, 25, 2, COLOR_PITCHSMP); display.fillRect( 210, 49, 27, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 54, 25, 2, COLOR_PITCHSMP); display.fillRect( 207, 54, 26, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 59, 25, 2, COLOR_PITCHSMP); display.fillRect( 204, 59, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 64, 25, 2, COLOR_PITCHSMP); display.fillRect( 202, 64, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 69, 25, 2, COLOR_PITCHSMP); display.fillRect( 200, 69, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 74, 25, 2, COLOR_PITCHSMP); display.fillRect( 199, 74, 24, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 79, 25, 2, COLOR_PITCHSMP); display.fillRect( 200, 79, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 84, 25, 2, COLOR_PITCHSMP); display.fillRect( 202, 84, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 89, 25, 2, COLOR_PITCHSMP); display.fillRect( 204, 89, 25, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 94, 25, 2, COLOR_PITCHSMP); display.fillRect( 207, 94, 26, 2, COLOR_PITCHSMP);
  display.fillRect( 291, 99, 25, 2, COLOR_PITCHSMP); display.fillRect( 210, 99, 27, 2, COLOR_PITCHSMP);
