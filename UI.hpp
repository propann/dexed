...
void lcdml_menu_control(void) {
  ...
  if (seq.gamepad_timer > 1000) {
    if (LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_automap_gamepad) && temp_int < 9) {
      ...
      // some pages do x/y navigation using ENC[R]for y and ENC[L] for x movement - depending on the edit state of the field, this needs special handling for gamepad usage

      if (LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_seq_pattern_editor) || LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_seq_vel_editor) || LCDML.FUNC_getID() < 58 || (LCDML.FUNC_getID() > 57 && LCDML.FUNC_getID() < 61) || (LCDML.FUNC_getID() > 61 && LCDML.FUNC_getID() < 89)) {
  ...
  if (LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_epiano))
    seq.gamepad_timer_speed = 3;
...
void UI_func_seq_vel_editor(uint8_t param) {
  ...
  if (LCDML.FUNC_loop())  // ****** LOOP *********
  {
    ...
    if (seq.note_data[seq.active_pattern][seq.menu - 1] > 0) {
      ...
      {
        if (seq.active_function == 1 && seq.content_type[seq.active_pattern] < 2) {  //if is Drum or normal Instrument Track
          if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up())) {
            if (LCDML.BT_checkDown())
        ...
        } else if (seq.active_function == 1 && seq.content_type[seq.active_pattern] > 1) {  //is in Chord or Arp Mode
          if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up())) {
            if (LCDML.BT_checkDown())
...
void sub_MultiSample_setColor(uint8_t row, uint8_t column) {
  ...
  if ((seq.temp_select_menu == row + 1 && seq.selected_track == column && seq.edit_state == false) || (seq.temp_select_menu == row + 1 && column == 99 && seq.edit_state == false)) {
    temp_color = COLOR_BACKGROUND;
    temp_background = COLOR_SYSTEXT;
  }
  else if ((seq.temp_select_menu == row + 1 && seq.selected_track == column && seq.edit_state ) || (seq.temp_select_menu == row + 1 && column == 99 && seq.edit_state )) {
    temp_background = RED;
...
void UI_func_MultiSamplePlay(uint8_t param) {
  ...
  {
    if ((LCDML.BT_checkDown() && encoderDir[ENC_R].Down()) || (LCDML.BT_checkUp() && encoderDir[ENC_R].Up())) {
      if (seq.edit_state && seq.temp_select_menu == 0) {
        if (LCDML.BT_checkDown()) {
      ...
      if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 6)  //file name selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 5)  //reverb send selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      } else if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 4)  //pan selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      } else if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 3)  //volume selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      } else if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 2)  //high selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      } else if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 1)  //low selection
      {
        if (LCDML.BT_checkDown()) {
      ...
      } else if (seq.edit_state && seq.temp_select_menu > 0 && seq.selected_track == 0)  //root note selection
      {
        if (LCDML.BT_checkDown()) {
    ...
    if (LCDML.BT_checkEnter()) {
      //if (seq.temp_select_menu == 0 && generic_active_function == 99 && seq.edit_state)
